...
const ProfileDoctor = ({ detail, setLoginStatus }) => {
        ...
        try {
            ...
            if (currProvider) {
                ...
                if (!existingPatient) {
                    ...
                    const c = await ContractInstance.methods.reportSendHandlerPopupDoctor(detail.licenseId, patientId, problem, desc, 100, 100).send({ from: userAccounts[0], gas: 3000000 });
    ...
    const onConnectfetchNotification = async () => {
        try {
            ...
            if (currProvider) {
                ...
                // const userAccounts = await web3.eth.getAccounts();
                const ContractInstance = new web3.eth.Contract(ABI, ADD);
                let notifiArray = await ContractInstance.methods.getNotificationPatient(detail.licenseId).call();
    ...
    const onConnectfetchReports = async () => {
        try {
            ...
            if (currProvider) {
                ...
                // const userAccounts = await web3.eth.getAccounts();
                const ContractInstance = new web3.eth.Contract(ABI, ADD);
                let reportArray = await ContractInstance.methods.getReportPatient(detail.licenseId).call();
                reportArray.reverse();
    ...
    return (
        <div className="profileDocWrapper">
            ...
            {
                resetPasswordPopupStatus && <PopupResetDoctorPassword onClose={setResetPasswordPopupStatus} id={detail.licenseId} />
            }
            <hr />
            <div className="profileNav">
                <div className="g1Doctor">Id:</div>
                <div className="g1Doctor" id="profileId">{detail.licenseId}</div>
                <div className="g1Doctor">Specialization:</div>
                <div className="g1Doctor" id="profileSpec">{detail.specialization}</div>
                ...
                <div className="g1Doctor" id="profileSpec">{parseInt(detail.contribution)}</div>
                <div className="g1Doctor">Rating:</div>
                <div className="g1Doctor" id="profileSpec">{parseInt(detail.finalRating) / 10}⭐</div>
                <div className="g1Doctor">Experience:</div>
                <div className="g1Doctor" id="profileSpec">{parseInt(detail.experience)} Yrs</div>
            </div>

            <div className="profileNav2">
                <div className="g1Doctor">Fees:</div>
                <div className="g1Doctor" id="profileSpec">Rs {parseInt(detail.fees)}</div>
                <div className="g1Doctor">Gender:</div>
                <div className="g1Doctor" id="profileSpec">{parseInt(detail.gender) ? "Male" : "Female"}</div>
                <div className="g1Doctor">Certified:</div>
                <div className="g1Doctor" id="profileSpec">{parseInt(detail.certified) ? "Yes" : "No"}</div>
            ...
            <div>
                ...
                {
                    approveStatus && <PopupApproveDoctor onClose={setApproveStatus} pid={pidApprove} did={detail.licenseId} indexPatient={indexPatient} indexDoctor={indexDoctor} />
                }
                {
                    reportPopupStatus && <ReportPopupDoctor onClose={setReportPopupStatus} pid={pidApprove} did={detail.licenseId} indexPatient={indexPatient} indexDoctor={indexDoctor} problem={inputProblem} />
                }
                {
